...
The [.productname]#OOPS# provides some functions that simplify the creation and use of projections.
...
In the future it may be added to the [.productname]#SQL# grammar, so that it is possible to write:
...
Like materialized views, [.productname]#OOPS# projections are not updated automatically.
...
The main advantage of [.productname]#OOPS# projection mechanism is that it allows to automatically substitute queries on original tables with projections.
...
By default it is disabled, because [.productname]#OOPS# projections may not be synchronized with the original table and a query on a projection may return different result.
