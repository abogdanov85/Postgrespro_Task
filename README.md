...
Для таких фрагментов я использовал класс (роль) `productname`, так как я заметил, что вы используете такой класс для некоторых коммерческих названий продуктов в вашей документации.
...
Я хочу подчеркнуть, что я на собственном опыте хорошо знаю, насколько плохо использовать одни и те же выделения для фрагментов разных типов в исходных документах.
...
И если использовать одинаковые выделения для таких разных случаев, то в будущем нас могут ждать неприятные последствия такого решения — например, если мы захотим для всех коммерческих названий сделать ссылки на их веб-страницы, то мы уже не сможем простым способом найти в документах только коммерческие названия, так как они будут смешаны с фрагментами другого типа, для которых ссылки не нужны ("**очень**", "**только**").
...
Поэтому в этой статье я для демонстрации этой идеи специально настроил css для фрагментов типа `productname` на обычный жирный шрифт.
...
В данном случае в словаре всего три входа, но он все равно полезен, так как мы в одном месте можем поменять тип выделения для всех вхождений объектов, которые в нем перечислены (например, изменить им роль, удалить ее, добавить url для ссылки и т.п.).
...
Помимо визуальных улучшений, специальное оформление блоков вывода может быть полезно для тестирования документации.
При сборке документации можно запускать выполнение всех блоков кода, для которых в документах есть блоки вывода, и сравнивать полученный после запуска вывод с зафиксированным в документе.
При несовпадении этих выводов можно либо останавливать сборку (и выводить ошибку с указанием блоков, для которых тест был неудачен), либо выводить предупреждения, но продолжать сборку.
Таким образом публикуемая документации может быть проверена на соответствие текущей версии продукта.
При сборке документации в языке Julia блоки вывода используются в том числе и для таких тестов.
